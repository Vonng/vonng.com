...
PostgreSQL 数据库中使用 UUIDv7 作为主键：
    ...
    --创建带 UUIDv7 主键的表
    CREATE TABLE users (
        id UUID PRIMARY KEYDEFAULT uuidv7(),
...
这样，每条新记录的 `id` 都会自动分配一个按时间戳排序的 UUID。
    ...
    -- 按 UUID 时间顺序查看
    SELECTid, username, uuid_extract_timestamp(id) as uuid_timestamp
    FROM users
    ORDER BY id;
...
#### 基础查询分析
    ...
    -- 查看增强的执行计划
    EXPLAIN ANALYZE
    SELECT *,sum(total_amount) OVER (PARTITIONBY customer_id)
...
执行计划：
    ...
    Insert on orders  (cost=0.00..2000.00 rows=0 width=0) (actual time=767.116..767.118 rows=0.00 loops=1)
       ...
       ->  Subquery Scanon"*SELECT*"  (cost=0.00..2000.00 rows=50000 width=28) (actual time=5.742..336.699 rows=50000.00 loops=1)
             Buffers: shared hit=50013
             WAL: records=1516 bytes=150084 buffers full=2
             ->  FunctionScanon generate_series n  (cost=0.00..1750.00 rows=50000 width=24) (actual time=5.460..227.650 rows=50000.00 loops=1)
...
[直播预告｜ PostgreSQL 与 IvorySQL 在云原生时代的演进与实践](https://mp.weixin.qq.com/s?__biz=Mzg2MDc0NDg1Mg==&mid=2247497624&idx=1&sn=11c424a90a0e89fb46127f9a9d42d7b1&scene=21#wechat_redirect)

[IvorySQL 文档共建计划第一期！提 PR，提 Issue，赢取 Beats 耳机、机械键盘、书籍等多重好礼！](https://mp.weixin.qq.com/s?__biz=Mzg2MDc0NDg1Mg==&mid=2247497604&idx=1&sn=318b0698a814d57a75b762cc2ac74e3e&scene=21#wechat_redirect)
